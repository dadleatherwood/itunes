    ...
    this.getSongData = function(artist) {
      ...
      .then(function(response){
        ...
        for (var i = 0; i < results.length;i++){
          var songInfo = results[i]
          var newSongData =
          {
            'Artist': songInfo.artistName,
            'Song': songInfo.trackName,
            'Collection': songInfo.collectionName,
            'AlbumArt': songInfo.artworkUrl100,
            'Type': songInfo.kind,
            'CollectionPrice': songInfo.collectionPrice,
            'Play': songInfo.previewUrl,
            'trackPrice': songInfo.trackPrice
          }
          filteredResults.push(newSongData)
        }
