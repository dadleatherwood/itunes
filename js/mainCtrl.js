angular.module('itunes').controller('mainCtrl', function($scope, itunesService){
  ...
  $scope.gridOptions = {
      ...
      showFilter: true
      // filterOptions: {filterText: "sortProp", useExternalFilter: false}
  };
  ...
  $scope.filterSongs = function() {
    if ($scope.sortProp === "all") {
      $scope.songData = $scope.allSongs.slice()
      return;
    }
    $scope.songData = $scope.allSongs.filter(function(song) {
        return song.Type === $scope.sortProp
    })
  }
